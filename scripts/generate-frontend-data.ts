...
function computeSecurityScore(): HealthSignal | null {
  ...
  const verdictMatch = content.match(
    /\*\*Verdict\*\*:\s*(CLEAN|NEEDS[\s_]*REVIEW|BLOCKING|NEEDS[\s_]*CLEANUP)/i,
  );
  const verdict = verdictMatch?.[1]?.toUpperCase() ?? "";
  const isClean =
    verdict === "CLEAN" || content.includes("0 vulnerabilities") || content.includes("0 vulns");
...
async function main() {
  ...
  const syncFiles = fs.existsSync(syncDir)
    ? fs
        .readdirSync(syncDir)
        .filter((f) => f.startsWith("sync-") && (f.endsWith(".yml") || f.endsWith(".yaml")))
    : [];
  ...
  const outFile = path.join(OUT, "project-data.ts");
  const rawTs = `// Auto-generated by scripts/generate-frontend-data.ts — do not edit
...
`;
  ...
  const ts = await prettier.format(rawTs, { ...prettierConfig, filepath: outFile });

  fs.writeFileSync(outFile, ts);
  console.log(`  Wrote ${path.relative(ROOT, outFile)}`);
  console.log(
    `  Stage: ${project.stage}, Health: ${data.healthSignals.length}, Risks: ${data.risks.length}, Sync: ${data.syncEvents.length}, Timeline: ${data.recentTimeline.length}`,
  );
}
