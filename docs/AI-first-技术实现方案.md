...
> **core 边界精确定义（二轮 P1-3）**：core **不写文件、不 spawn、不执行外部命令**。core 含两类函数：① 纯计算函数（结构化对象进、结构化对象出）；② repository reader 函数（只读本地 `.ai-first/` 文件，转为结构化对象）。git 命令、Codex 子进程、acceptance 命令、写文件——全部属于 runner/cli 副作用层，产出的客观事实（`GitBaseline` / `GitChangeSet` / `CodexRunResult` / `AcceptanceResult`）作为参数传入 core。这样单测、dry-run、CI fixture 都能稳定复现。
...
export function collectExecutionReport(params: {
  ...
  if (params.codexResult.timedOut)              { status = "blocked";        outcomeReason = "timeout"; }
  else if (params.codexResult.exitCode !== 0)   { status = "blocked";        outcomeReason = "non_zero_exit"; }
...
§4.5 时序里 preflight 阶段（工作区脏且未 `--allow-dirty`）要写 blocked report，但**此时尚未启动 Codex、也不该跑 acceptance-runner**，所以不能复用 `collectExecutionReport()`（它强制要 `codexResult` / `acceptanceResults`）。新增独立纯函数：
...
export function createPreflightBlockedReport(params: {
  ...
  return {
    ...
    scopeViolations: [],
    acceptanceResults: [],
    codexStdout: undefined,          // 无 Codex 执行，可空字段留 undefined
    codexStderr: undefined,
    codexExitCode: undefined,
...
主 collector 专注"Codex 已执行后"的归集，preflight 走独立入口，避免伪造 `CodexRunResult`。
...
npm run task:exec -- --task .ai-first/tasks/task-xxx.yml --runtime codex
  ...
  ├─ core/exec: collectExecutionReport({ baseline, changeSet, codexResult, acceptanceResults })（纯函数，§4.3）
...
> **F0 fixture 目录式结构（P2-3）**：每个样本是"录制事实 + 期望 report"——含 `CodexRunResult` / `GitBaseline` / `GitChangeSet` / `AcceptanceResult[]` 的录制 JSON + 一份 `expected-report.json`。CI 只断言 `collectExecutionReport(录制输入) ≈ expected-report`，**不依赖真实 Codex 或真实 git 工作区**。真实 Codex 采样仅作为零号批次手动 go/no-go，沉淀后才进 CI。
...
| 7 | F0 Codex report collector + fixture | `exec/report-collector-core.ts` + `fixtures/codex-output/` | ≥2 真实样本能生成 report（手动 go/no-go，不进 CI）；CI 只跑沉淀 fixture + 1 失败走降级 | 0b,0c,0d |
...
>
> **P2-3（命名）**：原"F0 Codex 解析器"改名为"Codex report collector"——因不再解析 Codex schema，而是工具侧收集。
>
...
| Codex report collector（F0） | fixture 驱动：`fixtures/codex-output/<sample>/`（录制的 `CodexRunResult` + `GitBaseline` + `GitChangeSet` + `AcceptanceResult[]`）→ `collectExecutionReport()` → 断言 status/filesChanged/scopeViolations/outcomeReason/taintedPaths |
